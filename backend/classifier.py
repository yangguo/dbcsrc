...
Choose the most appropriate category from the provided list."""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a text classification assistant. Always respond with valid JSON.",
                },
...
输入文本：{text}"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "你是一个专业的文本信息抽取助手。请严格按照要求以JSON格式返回结果。",
                },
                {"role": "user", "content": prompt},
            ],
            temperature=0.1,
    ...
    for i, (article, url) in enumerate(zip(artls, urls)):
        ...
        if analysis_result["success"]:
            ...
            result_row = {
                ...
                "作出处罚决定的机关名称": extracted_data.get(
                    "作出处罚决定的机关名称", ""
                ),
