...
class Metrics:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "error_rate": sum(self.error_count.values())
...
def get_mongo_client():
    ...
    try:
        ...
        safe_mongo_url = (
            mongo_url_parts[1] if len(mongo_url_parts) > 1 else mongo_url_parts[0]
        )
...
def get_online_data():
    ...
    try:
        ...
        if collection is not None:
            ...
            cursor = collection.find({}, {"_id": 0}).max_time_ms(
                15000
            )  # 15 second timeout
            data = list(cursor)
...
from web_crawler import (
    ...
    download_attachment,
    get_csrc2analysis,
    get_sumeventdf_backend,
    update_sumeventdf_backend,
)
...
async def global_exception_handler(request: Request, exc: Exception):
    logger.error(
        f"Global exception on {request.method} {request.url}: {str(exc)}", exc_info=True
    )
    return JSONResponse(
        status_code=500,
        content=APIResponse(
    ...
    try:
        ...
        is_healthy = all(
            [
                ...
                health_status["memory_usage"]["percent"]
                < 90,  # Less than 90% memory usage
            ]
        )
        ...
        return APIResponse(
            success=True, message="Health check completed", data=health_status
        )
    except Exception as e:
        logger.error(f"Health check failed: {str(e)}")
        return APIResponse(success=False, message="Health check failed", error=str(e))
...
async def get_metrics():
    ...
    try:
        ...
        return APIResponse(
            success=False, message="Failed to retrieve metrics", error=str(e)
        )
...
class ClassifyRequest(BaseModel):
    ...
    candidate_labels: List[str] = Field(
        ..., min_items=1, max_items=20, description="Classification labels"
    )
...
class AttachmentRequest(BaseModel):
    ...
    downloadFilter: str = Field(
        ..., min_length=1, description="Download filter criteria"
    )
...
class PenaltyAnalysisRequest(BaseModel):
    text: str = Field(
        ..., min_length=1, max_length=50000, description="行政处罚决定书文本内容"
    )
...
def get_summary_working(
    limit_orgs: int = Query(
        None, ge=1, le=100, description="Limit number of organizations (optional)"
    ),
    limit_months: int = Query(
        None, ge=1, le=60, description="Limit number of months (optional)"
    ),
):
        ...
        if not df.empty and total > 0:
            ...
            if "机构" in df.columns:
                try:
                    ...
                    if "发文日期" in df.columns:
                        ...
                        df_filtered = df.copy()
                        df_filtered["发文日期"] = pd.to_datetime(
                            df_filtered["发文日期"], errors="coerce"
                        )
                        df_filtered = df_filtered.dropna(subset=["发文日期", "机构"])
                        df_filtered = df_filtered[df_filtered["机构"].str.strip() != ""]

                        if not df_filtered.empty:
                            org_counts = df_filtered[
                                "机构"
                            ].value_counts()  # Show ALL organizations
                            if limit_orgs:
            ...
            if "发文日期" in df.columns:
                try:
                    ...
                    df_copy["发文日期"] = pd.to_datetime(
                        df_copy["发文日期"], errors="coerce"
                    )
                    ...
                    if not df_copy.empty:
                        ...
                        if limit_months:
                            month_counts = month_counts.tail(
                                limit_months
                            )  # Show last N months
...
def get_summary_fast():
    ...
    try:
        ...
        response = APIResponse(
            ...
            data={"total": 0, "byOrg": {}, "byMonth": {}, "timestamp": current_time},
        )
...
def get_org_comparison():
    ...
    try:
        ...
        if df.empty:
            return APIResponse(success=False, message="No data found", data={})
        ...
        if "机构" in df.columns:
            ...
            comparison_data["method_1_no_date_filter"] = {
                ...
                "top_10": {
                    org: f"{count} ({round(count/total_no_filter*100, 2)}%)"
        ...
        if "机构" in df.columns and "发文日期" in df.columns:
            ...
            if not df_filtered.empty:
                ...
                comparison_data["method_2_with_date_filter"] = {
                    ...
                    "top_10": {
                        org: f"{count} ({round(count/total_filtered*100, 2)}%)"
                ...
                diff_percentage = (
                    round((diff_total / total_no_filter) * 100, 2)
                    if total_no_filter > 0
                    else 0
                )
        ...
        return APIResponse(
            success=False, message="Organization comparison failed", error=str(e)
        )
...
def get_data_coverage():
    ...
    try:
        ...
        return APIResponse(
            success=True, message="Data coverage analysis completed", data=coverage_info
        )

    except Exception as e:
        logger.error(f"Data coverage analysis failed: {str(e)}", exc_info=True)
        return APIResponse(
            success=False, message="Data coverage analysis failed", error=str(e)
        )
    ...
    try:
        ...
        if not df.empty and "机构" in df.columns and "发文日期" in df.columns:
            ...
            if not df_copy.empty:
                ...
                logger.info(
                    f"Generated chart data for {len(org_counts)} organizations, total cases: {total_cases}"
                )
        ...
        logger.error(
            f"Error generating organization chart data: {str(e)}", exc_info=True
        )
        return APIResponse(
            success=False,
    ...
    try:
        ...
        if not df.empty and "机构" in df.columns and "发文日期" in df.columns:
            ...
            if not df_copy.empty:
                ...
                org_groups = org_groups.sort_values("caseCount", ascending=False).head(
                    50
                )
                ...
                for _, row in org_groups.iterrows():
                    org_data = {
                        ...
                        "minDate": (
                            row["minDate"].strftime("%Y-%m")
                            if pd.notna(row["minDate"])
                            else ""
                        ),
                        "maxDate": (
                            row["maxDate"].strftime("%Y-%m")
                            if pd.notna(row["maxDate"])
                            else ""
                        ),
                        "dateRange": (
                            f"{row['minDate'].strftime('%Y-%m')} 至 {row['maxDate'].strftime('%Y-%m')}"
                            if pd.notna(row["minDate"]) and pd.notna(row["maxDate"])
                            else "暂无数据"
                        ),
                    }
        ...
        logger.info(
            f"Generated organization summary for {len(org_summary)} organizations"
        )
...
def _get_summary_impl(limit_orgs: int = None, limit_months: int = None):
    try:
        ...
        if (
            ...
            and current_time - _summary_cache["timestamp"] < CACHE_DURATION
        ):
        ...
        if not df.empty:
            ...
            if "机构" in df.columns:
                ...
                if "发文日期" in df.columns:
                    ...
                    if not df_filtered.empty:
                        ...
                        logger.info(
                            f"Found {len(org_counts)} unique organizations (filtered for valid dates)"
                        )
                        if limit_orgs:
                            org_counts = org_counts.head(limit_orgs)
            ...
            if "发文日期" in df.columns:
                try:
                    ...
                    if not df_copy.empty:
                        df_copy["month"] = (
                            df_copy["发文日期"].dt.to_period("M").astype(str)
                        )
                        # Get ALL months and sort chronologically
                        month_counts = df_copy["month"].value_counts().sort_index()
                        logger.info(
                            f"Found {len(month_counts)} unique months from {month_counts.index.min()} to {month_counts.index.max()}"
                        )
                        if limit_months:
                            month_counts = month_counts.tail(
                                limit_months
            ...
            summary_data.update({"total": total, "byOrg": by_org, "byMonth": by_month})
        else:
        ...
        logger.info(
            f"Successfully generated summary: {total_cases} total cases, {online_cases} online cases"
        )
        ...
        return APIResponse(
            success=False, message="Failed to generate summary", error=str(e), data={}
        )
...
def search_cases(
    ...
    try:
        logger.info(
            f"Searching cases with filters: keyword={keyword}, org={org}, page={page}, pageSize={pageSize}"
        )
        ...
        if dateFrom:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="dateFrom must be in YYYY-MM-DD format"
                )
        ...
        if dateTo:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="dateTo must be in YYYY-MM-DD format"
                )
        ...
        # Apply filters
        if keyword:
            mask = df["标题"].str.contains(keyword, na=False, case=False) | df[
                "内容"
        ...
        if dateFrom:
            try:
                df = df[
                    pd.to_datetime(df["发文日期"], errors="coerce")
                    >= pd.to_datetime(dateFrom)
                ]
        ...
        logger.info(
            f"Search completed: returning {len(cases)} cases out of {total} total matches"
        )
        return SearchResponse(data=cases, total=total, page=page, pageSize=pageSize)
...
async def update_cases(request: UpdateRequest):
    ...
    try:
        logger.info(
            f"Starting update for organization: {request.orgName}, pages {request.startPage}-{request.endPage}"
        )

        # Get case summary data using backend functions
        logger.info(
            f"Fetching case data from pages {request.startPage} to {request.endPage}"
        )
        sumeventdf = get_sumeventdf_backend(
            request.orgName, request.startPage, request.endPage
        )

        if sumeventdf.empty:
            logger.warning(
                f"No data found for {request.orgName} in specified page range"
            )
            return APIResponse(
                success=True,
...
async def classify_text(request: ClassifyRequest):
    ...
    try:
        ...
        logger.info(
            f"Starting classification for text (length: {len(sanitized_article)} chars)"
        )
        ...
        if not sanitized_article or len(sanitized_article.strip()) == 0:
            ...
            return APIResponse(
                success=False, message="Text cannot be empty", error="Invalid input"
            )

        result = get_class(
            sanitized_article, request.candidate_labels, request.multi_label
        )
        ...
        return APIResponse(success=False, message="Classification failed", error=str(e))
...
async def batch_classify(
    ...
    try:
        ...
        logger.info(
            f"Batch classification completed successfully for {len(result_df)} records"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="Batch classification failed", error=str(e)
        )
...
async def analyze_attachments(request: AttachmentRequest):
    ...
    try:
        logger.info(
            f"Starting attachment analysis with contentLength={request.contentLength}, downloadFilter={request.downloadFilter}"
        )
        ...
        return APIResponse(
            success=False, message="Attachment analysis failed", error=str(e)
        )
...
async def download_attachments(request: AttachmentRequest):
    ...
    try:
        logger.info(
            f"Starting attachment download with contentLength={request.contentLength}, downloadFilter={request.downloadFilter}"
        )
        ...
        return APIResponse(
            success=False, message="Attachment download failed", error=str(e)
        )


@app.post("/amount-analysis", response_model=APIResponse)
async def amount_analysis(
    file: UploadFile = File(...), idcol: str = Query(...), contentcol: str = Query(...)
):
    ...
    try:
        ...
        logger.info(
            f"Amount analysis completed successfully for {len(result_df)} records"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="Amount analysis failed", error=str(e)
        )
...
async def location_analysis(
    ...
    try:
        ...
        logger.info(
            f"Location analysis completed successfully for {len(result_df)} records"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="Location analysis failed", error=str(e)
        )
...
async def people_analysis(
    ...
    try:
        ...
        logger.info(
            f"People analysis completed successfully for {len(result_df)} records"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="People analysis failed", error=str(e)
        )
...
async def penalty_analysis(request: PenaltyAnalysisRequest):
    ...
    try:
        ...
        return APIResponse(
            success=False, message="Penalty analysis failed", error=str(e)
        )
...
async def batch_penalty_analysis(
    ...
    try:
        ...
        logger.info(
            f"Batch penalty analysis completed successfully for {len(result_df)} records"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="Batch penalty analysis failed", error=str(e)
        )
...
async def convert_documents(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        logger.info(
            f"Document conversion completed successfully for {len(files)} files"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="Document conversion failed", error=str(e)
        )
...
async def refresh_data():
    ...
    try:
        ...
        logger.info(
            f"Data refresh completed successfully with {len(refreshed_cases)} cases"
        )
        return APIResponse(
            success=True,
        ...
        return APIResponse(
            success=False, message="Failed to refresh data", error=str(e), count=0
        )
...
async def get_download_data():
    ...
    try:
        ...
        data_dir = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "penalty", "csrc2"
        )
        ...
        def get_csv_stats(pattern, unique_id_column):
            ...
            if files:
                ...
                if dflist:
                    ...
                    unique_count = (
                        combined_df[unique_id_column].nunique()
...
async def download_case_detail():
    ...
    try:
        ...
        response = StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=case_detail_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
...
async def download_analysis_data():
    ...
    try:
        ...
        response = StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=analysis_data_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
...
async def download_category_data():
    ...
    try:
        ...
        response = StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=category_data_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
...
async def download_split_data():
    ...
    try:
        ...
        response = StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=split_data_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
...
async def get_upload_data():
    ...
    try:
        ...
        current_time = time.time()
        if upload_data_cache["data"] and (
            current_time - upload_data_cache["timestamp"] < upload_data_cache["expiry"]
        ):
        ...
        try:
            if not analysis_df.empty and not category_df.empty and not split_df.empty:
                logger.info(
                    f"Starting three-table intersection: analysis({len(analysis_df)}), category({len(category_df)}), split({len(split_df)})"
                )

                # Step 1: Inner join analysis with category data
                intersection_df = pd.merge(
                    analysis_df, category_df, left_on="链接", right_on="id", how="inner"
                )
                logger.info(
                    f"After analysis+category merge: {len(intersection_df)} records"
                )
                ...
                if not intersection_df.empty:
                    ...
                    logger.info(
                        f"After three-table intersection: {len(intersection_df)} records"
                    )
                    ...
                    if not online_df.empty and "链接" in online_df.columns:
                        ...
                        diff_df = intersection_df[
                            ~intersection_df["链接"].isin(online_case_ids)
                        ].copy()
                        logger.info(
                            f"After excluding online cases: {len(diff_df)} records"
                        )
                    else:
                        diff_df = intersection_df.copy()
                        logger.info(
                            f"No online data to exclude, keeping all intersection: {len(diff_df)} records"
                        )
                    ...
                    if selected_columns:
                        ...
                        logger.info(
                            f"Final diff data with selected columns: {len(diff_df)} records"
                        )
                else:
                ...
                logger.info(
                    f"Data availability: analysis={not analysis_df.empty}, category={not category_df.empty}, split={not split_df.empty}"
                )
                diff_df = pd.DataFrame()
        ...
        data = {
            "caseDetail": {
                "data": (
                    case_detail_df.to_dict("records")
                    if not case_detail_df.empty
                    else []
                ),
                "count": len(case_detail_df),
                "uniqueCount": (
                    case_detail_df["链接"].nunique()
                    if not case_detail_df.empty and "链接" in case_detail_df.columns
                    else 0
                ),
            },
            "analysisData": {
                ...
                "uniqueCount": (
                    analysis_df["链接"].nunique()
                    if not analysis_df.empty and "链接" in analysis_df.columns
                    else 0
                ),
            },
            "categoryData": {
            ...
            "splitData": {
                ...
                "uniqueCount": (
                    split_df["id"].nunique()
                    if not split_df.empty and "id" in split_df.columns
                    else 0
                ),
            },
            "onlineData": {
            ...
            "diffData": {
                ...
                "uniqueCount": (
                    diff_df["链接"].nunique()
                    if not diff_df.empty and "链接" in diff_df.columns
                    else 0
                ),
            },
        }
        ...
        # Create response
        response = APIResponse(
            success=True, message="Upload data retrieved successfully", data=data
        )
        ...
        return APIResponse(
            success=False, message="Failed to get upload data", error=str(e)
        )
...
async def upload_cases(request: UploadCasesRequest):
    ...
    try:
        ...
        if analysis_df.empty:
            return APIResponse(
                success=False, message="No analysis data available for upload", count=0
            )

        # Filter cases to upload
        if "案例编号" in analysis_df.columns:
            cases_to_upload = analysis_df[
                analysis_df["案例编号"].isin(request.case_ids)
            ]
        else:
        ...
        if cases_to_upload.empty:
            return APIResponse(
                success=False, message="No matching cases found for upload", count=0
            )
        ...
        if success:
            ...
            return APIResponse(
                success=False, message="Failed to upload cases to database", count=0
            )

    except Exception as e:
        logger.error(f"Case upload failed: {str(e)}", exc_info=True)
        return APIResponse(success=False, message="Case upload failed", error=str(e))
...
async def generate_labels():
    ...
    try:
        ...
        if analysis_df.empty:
            return APIResponse(
                success=False, message="No analysis data available for label generation"
            )
        ...
        category_update_df = analysis_df[
            analysis_df["链接"].isin(new_category_urls)
        ].copy()
        ...
        logger.info(
            f"Generated {total_cases} cases for labeling (Category: {len(label_data['category_cases'])}, Split: {len(label_data['split_cases'])})"
        )
        ...
        return APIResponse(
            success=False, message="Label generation failed", error=str(e)
        )
...
async def delete_online_data_endpoint():
    ...
    try:
        ...
        return APIResponse(
            success=False, message="Online data deletion failed", error=str(e)
        )
...
async def download_online_data():
    ...
    try:
        ...
        if online_df.empty:
            ...
            raise HTTPException(
                status_code=404, detail="No online data available for download"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"CSV conversion failed: {str(csv_error)}"
            )
        ...
        try:
            response = StreamingResponse(
                ...
                media_type="text/csv",
                headers={
                    "Content-Disposition": f"attachment; filename=online_data_{datetime.now().strftime('%Y%m%d')}.csv"
                },
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to create response: {str(resp_error)}"
            )
...
async def download_diff_data():
    ...
    try:
        ...
        if not analysis_df.empty:
            ...
            if not online_df.empty and "链接" in online_df.columns:
                online_case_ids = set(online_df["链接"].tolist())
                diff_analysis = analysis_df[
                    ~analysis_df["链接"].isin(online_case_ids)
                ].copy()
            else:
        ...
        response = StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=diff_data_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
