...
class DBCSRCUser(HttpUser):
    ...
    def detailed_health_check(self):
        ...
        with self.client.get("/health/detailed", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(f"Detailed health check failed with status {response.status_code}")
    ...
    def classify_text(self):
        ...
        with self.client.post("/classify", json=request_data, catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
                if data.get("success") and "data" in data:
                    response.success()
    ...
    def batch_classify(self):
        ...
        with self.client.post("/batch-classify", json=request_data, catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
                if data.get("success") and "data" in data:
                    response.success()
                ...
                response.failure(f"Batch classification failed with status {response.status_code}")
    ...
    def amount_analysis(self):
        ...
        with self.client.post("/amount-analysis", json=request_data, catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
                if data.get("success"):
                    response.success()
                ...
                response.failure(f"Amount analysis failed with status {response.status_code}")
    ...
    def location_analysis(self):
        ...
        with self.client.post("/location-analysis", json=request_data, catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
                if data.get("success"):
                    response.success()
                ...
                response.failure(f"Location analysis failed with status {response.status_code}")
    ...
    def people_analysis(self):
        ...
        with self.client.post("/people-analysis", json=request_data, catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
                if data.get("success"):
                    response.success()
                ...
                response.failure(f"People analysis failed with status {response.status_code}")
    ...
    def test_invalid_requests(self):
        ...
        # Test invalid JSON
        with self.client.post("/classify", data="invalid json", 
                             headers={"Content-Type": "application/json"}, 
                             catch_response=True) as response:
            if response.status_code in [400, 422]:
                response.success()  # Expected error response
            else:
                response.failure(f"Expected 400/422 for invalid JSON, got {response.status_code}")
        ...
        with self.client.post("/classify", json={}, catch_response=True) as response:
            if response.status_code in [400, 422]:
                ...
                response.failure(f"Expected 400/422 for missing fields, got {response.status_code}")
...
def on_test_stop(environment, **kwargs):
    ...
    if stats.total.num_failures / stats.total.num_requests > 0.01:  # 1% error rate threshold
...
if __name__ == "__main__":
    print("Use 'locust -f performance_tests.py --host=http://localhost:8000' to run tests")
    print("\nExample commands:")
    print("  # Web UI mode:")
    print("  locust -f performance_tests.py --host=http://localhost:8000")
    print("\n  # Headless mode:")
    print("  locust -f performance_tests.py --headless --users 50 --spawn-rate 5 --run-time 60s --host=http://localhost:8000")
    print("\n  # Stress test:")
    print("  locust -f performance_tests.py --headless --users 100 --spawn-rate 10 --run-time 120s --host=http://localhost:8000 StressTestUser")